...
EXAMPLE_COMMAND = 'do'
CHUG_COMMAND = 'chug gas'
IMMOLATION_COMMAND = 'self-immolate'
MENTION_REGEX = "^<@(|[WU].+?)>(.*)"
...
def parse_bot_commands(slack_events):
    ...
    for event in slack_events:
        if (event["type"] == "member_joined_channel"
                and event["user"] == starterbot_id):
                    handle_command("just_joined", event["channel"])
...
def handle_command(command, channel):
    ...
    response = None
    # This is whre you start to implement more commands!
    if command == "just_joined":
        response = "Hello"
    elif command.startswith(EXAMPLE_COMMAND):
        response = "Sure... write some more code then I can do that!"
    elif command.startswith(CHUG_COMMAND):
        response = "`*gulp* *gulp* *gulp*`"
    elif command.startswith(IMMOLATION_COMMAND):
        response = "`*Lights self on fire*` I'm burnin' baby!"
